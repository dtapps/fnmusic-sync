 ...
 */
// ConvertPathItem trim.file.convertPath 返回的单个路径条目
interface ConvertPathItem {
  path?: string;
  semanticPath?: string;
}

// ConvertPathResponse trim.file.convertPath 的返回信封（兼容多种结构）
interface ConvertPathResponse {
  data?: { result?: unknown; data?: { result?: unknown } };
  result?: unknown;
