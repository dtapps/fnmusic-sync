...
func (s *Server) handleLibrary(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut, http.MethodPost:
		...
		if err := config.SaveLibrary(s.configPath, cleaned, req.ScanInterval); err != nil {
		...
		s.logger.Info("音乐目录配置已通过 Web UI 保存", "目录数", len(cleaned), "扫描间隔", req.ScanInterval)
		writeJSON(w, http.StatusOK, map[string]any{"ok": true, "directories": cleaned, "scan_interval": req.ScanInterval})
