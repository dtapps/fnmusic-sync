...
- **Last.fm 智能歌单同步**：基于 scrobble 数据自动生成 top_tracks（最常听）、loved_tracks（红心收藏）、recent_tracks（最近播放）、weekly_charts（本周榜单）歌单。
...
| `playlist`                                              | 歌单同步总开关与间隔                                                                                                                          |
| `users.<name>.lastfm.playlist`                          | Last.fm 智能歌单同步（top_tracks / loved_tracks / recent_tracks / weekly_charts）                                                             |
