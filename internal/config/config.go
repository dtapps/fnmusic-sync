...
type UserPlaylistConfig struct {
	...
	// LovedTracks 喜欢的录音歌单（ListenBrainz loved recordings）。
	LovedTracks PlaylistSourceConfig `mapstructure:"loved_tracks" json:"loved_tracks"`
...
func userAccountToMap(u UserAccount) map[string]any {
	...
	lbMap := map[string]any{
		...
		"playlist": map[string]any{
			...
			"loved_tracks": map[string]any{
				"enabled": u.ListenBrainz.Playlist.LovedTracks.Enabled,
				"name":    u.ListenBrainz.Playlist.LovedTracks.Name,
				"limit":   u.ListenBrainz.Playlist.LovedTracks.Limit,
			},
