...
// GetLibraryTracks 获取用户曲库曲目（Last.fm library.getTracks，基于 scrobble 历史）。
// limit 最多返回的曲目数（Last.fm 上限约 1000），0 表示不限制（取上限 1000）。
// 返回按播放次数排序的歌单；接口无数据时返回空歌单（TrackList 为空）。
func (c *Client) GetLibraryTracks(ctx context.Context, username string, limit int) (*Playlist, error) {
	if username == "" {
		return nil, fmt.Errorf("用户名不能为空")
	}
	// library.getTracks 是需 session 鉴权的方法（不同于 user.getTopTracks 等公开接口），
	// 缺少 session key 时 Last.fm 直接返回 400，这里提前给出明确提示。
	if c.SessionKey == "" {
		return nil, fmt.Errorf("last.fm Library 需要授权（session key 缺失），请先在设置中完成 Last.fm 授权")
	}
	if limit <= 0 || limit > 1000 {
		limit = 1000
	}
	...
	params := url.Values{}
	params.Set("method", "library.getTracks")
	params.Set("api_key", c.APIKey)
	params.Set("sk", c.SessionKey)
	params.Set("user", username)
	params.Set("limit", strconv.Itoa(limit))
	// 鉴权方法需带 api_sig（按 Last.fm 规则对除 format/callback 外的参数签名）。
