...
func New(dir, filename string, maxSize, maxBackups, maxAge int, compress bool, enabled *atomic.Bool) *Logger {
	...
	l.wg.Add(1)
	safego.Go(nil, "reqlog.worker", l.worker)
...
// 仅负责 reqlog 自身的三个日志（feiniu.log / lastfm.log / listenbrainz.log）；
// capture.log 的头部打码在 proxy 包内自行处理，不在此处。
...
// sensitiveKeys 需要打码的敏感参数名（大小写不敏感）。
// 单一来源（single source of truth），同时用于查询串 / form 体 与 JSON / XML 响应体的打码，
// 避免 key / 会话密钥等敏感信息以明文落盘到 lastfm.log / listenbrainz.log / feiniu.log。
// 注意：这里只是“要打码哪些键”的声明，打码逻辑见下方 redactKVPairs / redactSensitiveText，
// 并非“删除打码”——任何对这些键的赋值在日志里都会被替换成 ***。
var sensitiveKeys = []string{
	"api_key", "apikey",
	"api_secret", "apisecret",
	"secret",
	"sk",
	"sessionkey", "session_key", "session",
	"password", "passwd", "pass", "pwd",
	"token", "auth_token", "access_token", "refresh_token",
	"api_sig", "signature", "sig",
	"key",
}

// sensitiveKVRe 匹配 "key=value" 形式的敏感参数（大小写不敏感），
// 分隔符可为行首或 &/?。用于在原始查询串 / form 体上直接打码。
// 注意：不能在打码后再用 url.Values.Encode() 之类重新编码，否则 "***" 会被
// 二次编码成 %2A%2A%2A（上一版就是这错）。这里只在原始串上替换占位符，保持原编码。
var sensitiveKVRe = regexp.MustCompile(`(?i)((?:^|[?&]))(` + strings.Join(sensitiveKeys, "|") + `)=([^&]*)`)

// redactKVPairs 把敏感 "key=value" 的值直接替换为 "***"，不重新编码。
func redactKVPairs(s string) string {
	return sensitiveKVRe.ReplaceAllString(s, `$1$2=***`)
}

// RedactQuery 将查询串中敏感参数值打码；保留原编码，不二次编码。
func RedactQuery(raw string) string {
	if raw == "" {
		return raw
	}
	return redactKVPairs(raw)
}

// RedactPath 对 path / URL 中携带的查询串打码。
// 有些调用把整个 URL 当 path 传进来（如歌单同步的 GET ...?api_key=xxx&...），
// 因此这里也要把 "?" 之后的查询参数掩一遍，避免 key 从请求行泄漏。
func RedactPath(p string) string {
	...
	return before + "?" + RedactQuery(after)
}
...
func RedactBody(contentType string, body []byte) []byte {
	...
	if strings.Contains(strings.ToLower(contentType), "x-www-form-urlencoded") {
		return []byte(redactKVPairs(string(body)))
	}
...
// 键名列表与 sensitiveKeys 共用（单一来源），改名单只改上面一处。
var (
	reJSONSecret = regexp.MustCompile(`(?i)("(?:` + strings.Join(sensitiveKeys, "|") + `)"\s*:\s*")([^"]*)(")`)
	reXMLSecret  = regexp.MustCompile(`(?i)<(` + strings.Join(sensitiveKeys, "|") + `)[^>]*>([^<]*)</(?:` + strings.Join(sensitiveKeys, "|") + `)>`)
)
