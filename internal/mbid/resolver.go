...
type ScanStats struct {
	...
	HasMBID   int    `json:"has_mbid"`   // 标签里实际解析到任一 MBID 的文件数（诊断用）
	Online    int    `json:"online"`     // 本地无 MBID 但经 MusicBrainz 在线补全成功的数目
...
func NewResolver(cfg *config.Config, store *datadb.Store, lister TrackLister, logger *slog.Logger) *Resolver {
	return &Resolver{cfg: cfg, store: store, lister: lister, mbClient: NewMusicBrainzClient(), logger: logger}
}
...
func (r *Resolver) Scan(ctx context.Context) (ScanStats, error) {
	...
	// onlineCache 单次扫描内的「艺人名+曲名 → 录音MBID」查询缓存，
	// 同一首歌在多个文件里出现时只查一次 MusicBrainz（省请求、降限速压力）。
	onlineCache := map[string]string{}
	...
	for _, dir := range r.cfg.Library.Directories {
		...
		walkErr := filepath.WalkDir(absDir, func(path string, d os.DirEntry, werr error) error {
			...
			if !isZeroMBID(m) {
				stats.HasMBID++ // 诊断：该文件标签里确实含 MBID
			}
			...
			// 本地标签含 MBID：直接落库（matched_by=tag）。
			if !isZeroMBID(m) {
				stats.HasMBID++
				r.saveMBID(&stats, datadb.TrackMBID{
					FeiniuGUID:       guid,
					FilePath:         path,
					Title:            m.Title,
					Artist:           m.Artist,
			...
			// 本地无 MBID：若开启在线补全，则查询 MusicBrainz 获取录音 MBID（matched_by=musicbrainz）。
			if r.cfg.Library.MBIDOnlineLookup {
				if id, ok := r.resolveOnlineMBID(ctx, onlineCache, m.Artist, m.Title); ok {
					stats.Online++
					r.saveMBID(&stats, datadb.TrackMBID{
						FeiniuGUID:    guid,
						FilePath:      path,
	...
	r.logger.Info("MBID 扫描完成",
		...
		"含MBID", stats.HasMBID, "在线", stats.Online, "跳过", stats.Skipped, "错误", stats.Errors, "耗时(ms)", stats.ElapsedMs,
	)
	return stats, nil
}
...
// saveMBID 落库一条 MBID 映射，并维护扫描统计（更新数 / 错误数）。
func (r *Resolver) saveMBID(stats *ScanStats, rec datadb.TrackMBID) {
	if serr := r.store.SaveTrackMBID(rec); serr != nil {
		stats.Errors++
		r.logger.Warn("MBID 保存失败", "路径", rec.FilePath, "错误", serr)
		return
	}
	stats.Updated++
}

// resolveOnlineMBID 在线查询 MusicBrainz 补全录音 MBID（带单次扫描内缓存）。
// 返回 ("", false) 表示未查到 / 查询被限流 / 网络错误（调用方应跳过该曲）。
func (r *Resolver) resolveOnlineMBID(ctx context.Context, cache map[string]string, artist, title string) (string, bool) {
	key := strings.ToLower(artist) + "\x00" + strings.ToLower(title)
	if v, ok := cache[key]; ok {
		return v, v != ""
	}
	id, ok := r.mbClient.RecordingMBIDByArtistTitle(ctx, artist, title)
	if ok {
		cache[key] = id
	} else {
		cache[key] = "" // 记空，避免同次扫描内对同曲重复请求
	}
	return id, ok
}
