...
常用命令：
  ...
  __BINARY_NAME__ version        查看版本、Git 提交与构建时间
  sudo __BINARY_NAME__ service install
                              安装 systemd 服务（kardianos/service，需 root）
  sudo systemctl enable --now __BINARY_NAME__
  sudo __BINARY_NAME__ self-upgrade
                              升级到 CNB 仓库最新版本（写 /usr/bin 需 sudo）

示例：
  sudo __BINARY_NAME__ self-upgrade
  __BINARY_NAME__ --config /etc/fnmusic-sync/config.yaml --state /var/lib/fnmusic-sync/state.yaml
EOF
